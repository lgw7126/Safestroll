...
export default function AlertScreen({ walk, contact, onCheckIn }) {
  const hasContact = contact?.name || contact?.phone

  return (
    <div className="screen screen--alert">
      <div className="alert-banner">
        <div className="alert-banner__kr">
          귀가 예정 시간이 지났습니다
        </div>
        <div className="alert-banner__en">
      ...
      {hasContact ? (
        <div className="contact-card">
          ...
          {contact.phone && (
            <a className="call-link" href={`tel:${contact.phone}`}>
              전화하기 · Call
            </a>
          )}
        </div>
        ...
        <div className="notice">
          ...
          홈 화면에서 비상연락처를 설정하면 이 상황에서 자동으로 알림이 갑니다.<br />
          <span style={{ fontSize: '0.8rem' }}>
            No emergency contact set. Add one from the home screen.
          </span>
        </div>
      )}
